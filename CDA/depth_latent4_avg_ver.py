...
class depth_latent4_avg_ver(nn.Module):
    def __init__(
        self,
        encoder='vits', 
        features=64, 
        ...
        super(depth_latent4_avg_ver, self).__init__()
        print("depth_latent4_avg_ver")
        ...
        self.teacher_pretrained = DINOv2(model_name=self.teacher_encoder)
    ...
    def forward(self, x):
        ...
        teacher_depth_features = self.teacher_pretrained.get_intermediate_layers(x, self.intermediate_layer_idx[self.teacher_encoder], return_class_token=False)
        teacher_depth_features = torch.stack(teacher_depth_features).mean(dim=0)

        proj_feature = []
        for student_feat, Channel_CrossAttn, CrossAttn, SelfAttn in zip(depth_features,self.Projects_layers_Channel_based_CrossAttn_Block,self.Projects_layers_Cross, self.Projects_layers_Self):
            student_feat = student_feat
            channel_proj_feat = Channel_CrossAttn(student_feat,teacher_depth_features)
            feat = CrossAttn(student_feat,channel_proj_feat)
            feat = SelfAttn(feat)
    ...
    def freeze_depth_latent4_style(self):
