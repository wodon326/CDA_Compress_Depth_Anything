...
class DinoVisionTransformer(nn.Module):
    ...
    def _get_intermediate_layers_not_chunked_start_intermediate(self, x, n=1):
        ...
        output, total_block_len = [], len(self.blocks)
        blocks_to_take = range(total_block_len - n, total_block_len) if isinstance(n, int) else n
        start_block = blocks_to_take[0]+1
        for i, blk in enumerate(self.blocks[start_block:]):
            x = blk(x)
            if i+start_block in blocks_to_take:
                output.append(x)
    ...
    def unfreeze_last_n_intermediate_layer(self, idx, n):
        for i, blk in enumerate(self.blocks):
            if i in idx[-1*n:]:
                for i, (name, param) in enumerate(blk.named_parameters()):
